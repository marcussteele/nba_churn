...
def change_type_data(df):
    ...
    df['Yrs'] = df['Yrs'].astype('category')
    df['Type'] = df['Type'].astype('category')

def drop_duplicates_useless(df):
    df.drop(['Avg. Salary','Pos.','To','From','Tm','Year of Free Agency','Year'],axis=1,inplace=True)
    df.drop(['3P%','2P%','FT%','FG%','eFG%'],axis=1,inplace=True)
    df_no_nan = df[~(df['MP'].isna())&~(df['Salary'].isna())]
    final_df = df_no_nan.drop(['Pos','Yrs','Type'],axis=1)
    return final_df
